...
<?php
	...
	$class_id=$_GET['cN'];
	if(!in_array($class_id,$classes)) die( "No such record. 18" );

	$classNode=new Node;
	$node=$classNode->getObjectByUid($class_id) or die("No such record 21");
...
<head>
  ...
  <title><?php echo $name. ' - '.$node->getCode().'  '.$node->getYear(); ?></title>
...
<!-- Fixed navbar -->
    ...
    <div class="container">
	    <?php
	    	...
	    	echo '<h3>Class : '.$node->getCode().' '.($node->getYear()).'</h3>';
	    	echo '<h3>Number of Classes conducted : '.$node->getDays().'</h3>';
	    	echo '<button class="btn btn-primary" data-toggle="modal" data-target=".bs-example-modal-sm">Help me!</button>  <button id="submit" class="btn btn-success">Save</button>';
	    ?>
	    <div id="studentRecords">
	    	...
	    	foreach($records as $roll=>$data){
	        	...
	        	echo "<div class='student-record'><span class='roll'><a href='student.php?roll=$roll&code=$code&year=$year'>$roll</a></span>:<span class='present'>$present</span><button class='marker btn' style='margin-left:20px;'>A</button></div>";
	    	}
	    	?>
	    </div>
